...
const getById = async (id) => {
  try {
    const data = await PaymentType.findById(id);
    if (!data) {
...
const create = async (newData) => {
  try {
    const data = new PaymentType(newData);
    await data.save();
    return data;
  } catch (error) {
...
const updateById = async (id, updateData) => {
  try {
    const data = await PaymentType.findByIdAndUpdate(id, updateData, { new: true });
    if (!data) {
...
const deleteById = async (id) => {
  try {
    const data = await PaymentType.findByIdAndDelete(id);
    if (!data) {
