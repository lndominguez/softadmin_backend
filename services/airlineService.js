...
const getById = async (id) => {
  try {
    const data = await Airline.findById(id);
    if (!data) {
...
const create = async (newData) => {
  try {
    const data = new Airline(newData);
    await data.save();
    return data;
  } catch (error) {
...
const updateById = async (id, updateData) => {
  try {
    const data = await Airline.findByIdAndUpdate(id, updateData, { new: true });
    if (!data) {
...
const deleteById = async (id) => {
  try {
    const data = await Airline.findByIdAndDelete(id);
    if (!data) {
